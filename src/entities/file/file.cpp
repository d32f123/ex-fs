...
int file::trunc(std::size_t new_size)
{
    ...
    const auto block_size_bytes = fs_->super_block_.block_size * SECTOR_SIZE;
    auto free_blocks = new_size / block_size_bytes + (new_size % block_size_bytes != 0);
...
int file::write_unaligned(const uint32_t start_block, std::size_t offset, const std::size_t obj_size, const void * buffer)
{
    const auto block_size_bytes = fs_->super_block_.block_size * SECTOR_SIZE;
	std::size_t obj_pos = 0;
	uint32_t i = start_block;
...
int file::allocate_block(const uint32_t block_index)
{
    ...
    else if (block_index < INODE_BLOCKS_MAX + indirect_max)
    {
        ...
        if (temp == 0)
        {
            ...
            ret = fs_->write_data_object(inode_.indirect_block, (block_index - INODE_BLOCKS_MAX) * sizeof(uint32_t), sizeof(uint32_t), &free_block);
			if (ret < 0)
				return ret;
